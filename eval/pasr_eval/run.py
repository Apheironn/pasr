"""Run a PASR evaluation plan end to end and write the delivery.
    ...
    pasr-bench run --agent keyword                       # offline smoke, no API
    ANTHROPIC_API_KEY=... pasr-bench run --agent claude   # the real run
...
from pasr_eval import load_plan, run_plan, validate_matrix
from pasr_eval.arms import ArmResult
from pasr_eval.metrics import full_report
from pasr_eval.runner import resolve_repos

_PLANS_DIR = Path(__file__).parent / "plans"
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--plan", default=str(_PLANS_DIR / "pilot.json"))
    parser.add_argument("--out", default="pasr-bench-runs")
